...
const CARDS: readonly HubCardConfig[] = [
  ...
  {
    description: ADMIN_HUB_SCREEN.REPORTS.DESCRIPTION,
    href: PATHS.ADMIN.REPORTS,
    icon: MATERIAL_ICON_NAME.DASHBOARD,
    title: ADMIN_HUB_SCREEN.REPORTS.TITLE,
  },
];

/**
 * `/administracion` — a small hub over every section of the module
 * (EP-ADM-01 through EP-ADM-06). Large, well-separated tap targets at
 * every width (US-TAB-005's rule, carried into this module).
 */
    ...
    <main className="mx-auto grid max-w-6xl grid-cols-1 gap-md sm:grid-cols-2">
      {CARDS.map((card) => (
        <Link
          key={card.href}
          href={card.href}
          className="flex min-h-32 flex-col justify-between gap-md rounded-xl border border-outline-variant bg-surface-container/40 p-md backdrop-blur-md transition-colors hover:bg-surface-container-high/60"
        >
