 ...
 *
 * US-ACC-011: every mode-sensitive decision below reads `activeArea`, the
 * mode this worker actually chose, never `areas` directly — an account
 * holding both reservas and operaciones must see the operaciones-only
 * calendar while working in operaciones mode, exactly as if it had no
 * other area.
 */
const CalendarPage = async ({
  ...
  const isReservationsMode =
    hasReservationsModeAccess(activeArea);
  const allowedViews = isReservationsMode
    ? ALL_CALENDAR_VIEWS
    : OPERATIONS_CALENDAR_VIEWS;
  const defaultView = isReservationsMode
    ? DEFAULT_CALENDAR_VIEW
    : DEFAULT_OPERATIONS_CALENDAR_VIEW;
  ...
  return (
    <Calendar
      ...
      canCreate={isReservationsMode}
      canCreateExternalGuide={
        isExternalGuideRegistrar && isReservationsMode
      }
      range={range}
